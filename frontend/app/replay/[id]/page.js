...
export default function ReplayViewerPage() {
  ...
  if (loading) {
    return (
      <div className="w-full min-h-screen bg-black flex items-center justify-center">
        <span className="text-white/20 text-sm">Loading replay...</span>
      </div>
    )
  }
  ...
  if (!replay) {
    return (
      <div className="w-full min-h-screen bg-black flex items-center justify-center">
        <span className="text-white/30 text-sm">Replay not found</span>
      </div>
    )
  }
    ...
    <div className="w-full min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <a href="/replays" className="flex items-center gap-2 text-white/30 hover:text-white/60 transition-colors text-sm">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 gap-8">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={stepBack} className="text-white/20 hover:text-white/50 transition-colors">
                <SkipBack size={18} />
              </button>
              <motion.button
                onClick={togglePlay}
                ...
                whileTap={{ scale: 0.95 }}
                className="w-12 h-12 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-white/60 hover:bg-white/15 transition-colors"
              >
                {isPlaying ? <Pause size={18} /> : <Play size={18} className="ml-0.5" />}
              </motion.button>
              <button onClick={stepForward} className="text-white/20 hover:text-white/50 transition-colors">
                <SkipForward size={18} />
              </button>
            </div>
            ...
            <div className="w-80 flex items-center gap-3">
              <span className="text-white/20 text-[10px] font-mono w-6 text-right">{currentTurn}</span>
              ...
              <span className="text-white/20 text-[10px] font-mono w-6">{replay.moves.length}</span>
            </div>
          ...
          {/* Move list */}
          <div className="w-80 bg-white/[0.03] border border-white/10 rounded-xl p-4 max-h-32 overflow-y-auto">
            <div className="space-y-0.5">
              {replay.moves.map((m, i) => (
                <div
                  ...
                  } ${i === currentTurn - 1 ? "bg-white/5" : ""}`}
                >
                  <span className="text-white/30">{m.turn}.</span>
                  <span className={m.player === "human" ? "text-red-400/60" : "text-yellow-400/60"}>
                    {m.player}
                  </span>
                  <span className="text-white/20">col {m.col + 1}, row {m.row + 1}</span>
                </div>
              ))}
            </div>
          </div>
